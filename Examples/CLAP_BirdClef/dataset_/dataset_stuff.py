...
from dataset_.BasicDatasetClasses import FastDataLoader, BasicTrainingSet, BasicVariableRawToSpecSet
...
def my_collate(batch):
    """
    Handles the triple-output for Task-Adaptive Meta-Learning.
    Ensures raw audio and spectrograms stay synchronized in the batch.
    """
    # batch is [(spec, raw, label), (spec, raw, label), ...]
    specs = torch.stack([torch.from_numpy(item[0]) if isinstance(item[0], np.ndarray) else item[0] for item in batch])
    raws = torch.stack([torch.from_numpy(item[1]) if isinstance(item[1], np.ndarray) else item[1] for item in batch])
    labels = torch.tensor([item[2] for item in batch])

    return specs, raws, labels
...
def gen_training_stats(data_path, training_classes, norm, type, save_path, num_workers, params):
    ...
    if type in ['spec', 'raw']:
        dataloader = FastDataLoader(dataset, num_workers=num_workers, batch_size=1000)
    elif type == 'variable_spec':
