...
def eval_step_zero_shot_var(clap_model, batch_wavs, q_num, y, batch_class_names,
    ...
    """
    Performs a Zero-Shot evaluation episode using Frozen CLAP.
    Support audio is ignored; text anchors are used as the sole class prototypes.
    """
    clap_model.eval()
    ...
    for idx in range(meta_batch_size):
        ...
        with torch.no_grad():
            t_k = clap_model.get_text_anchors(ep_classes) # [n_way, 512]

        # 完美切片：跳过 Support，直接提取对应的 n_way * q_queries 个 Query 波形
        wav_task_val = ep_wavs[(k_shot * n_way) : (k_shot * n_way) + queries_per_ep]
        # 提取 Ground Truth 并送入对应设备
        y_task_val = y[idx][(n_way * k_shot):].to(device)

        with torch.no_grad():
            z_q = clap_model.get_audio_features(wav_task_val) # [num_queries, 512]
            logits = z_q @ t_k.T # [num_queries, n_way]

        # 抛弃 majority_vote，直接取最大概率索引
